...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  // 이미지 preload를 통해 브라우저 캐싱 활용 (remount 시 재요청 방지)
  useEffect(() => {
    if (!isMounted || !images.length) return;

    // 고유한 이미지 src만 preload (중복 제거)
    const uniqueImageSrcs = Array.from(new Set(images.map((img) => img.src)));

    uniqueImageSrcs.forEach((src) => {
      // 이미 preload된 이미지는 건너뛰기
